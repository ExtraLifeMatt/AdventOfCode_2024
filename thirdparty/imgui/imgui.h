...
namespace ImGui
{
    ...
    // Temporary Frame Memory Allocators. By default these will fallback to the normal allocators, but they are ideal for ring buffers.
    // Any memory allocated with this function is assumed to only be valid till the next call of ImGui::NewFrame. So don't store any pointers for more than a frame.
    IMGUI_API void          SetFrameAllocatorFunctions(ImGuiMemAllocFunc frame_alloc_func, ImGuiMemFreeFunc frame_free_func);
    IMGUI_API void*         FrameMemAlloc(size_t size);
    IMGUI_API void          FrameMemFree(void* ptr);
...
struct ImTextParsedTag
{
    ImGuiID tagId;
    const char* tagNameStart = nullptr;
    const char* tagNameEnd = nullptr;
    const char* attributeStart = nullptr;
    const char* attributeEnd = nullptr;
    const char* rangeStart = nullptr;
    const char* rangeEnd = nullptr;
    const ImTextCustomizationTag* tagCustomization = nullptr;
...
struct ImTextCustomization
{
    ...
    static bool ParseIntValue(const char* strStart, const char* strEnd, const char* paramName, int& outValue); // Supports Hex
    static bool ParseFloatValue(const char* strStart, const char* strEnd, const char* paramName, float& outValue);
    static bool ParseStringValue(const char* strStart, const char* strEnd, const char* paramName, const char* strValueStart, const char* strValueEnd);
    static bool ParseBoolValue(const char* strStart, const char* strEnd, const char* paramName, bool& outValue); // Supports 0, 1, True, False
    ...
    // Used during draw to prevent creating and destroying vectors on the heap.
    ImVector<ImTextParsedTag> parsedTagVec;
    ImVector<const ImTextParsedTag*> parsedTagsByRenderOrder;
};
